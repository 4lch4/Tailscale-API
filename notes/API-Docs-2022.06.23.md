...
Deletes the provided device from its tailnet. The device must belong to the user's tailnet. Deleting shared/external devices is not supported.
...
```
curl 'https://api.tailscale.com/api/v2/device/11055/routes' -u "tskey-yourapikey123:"
```
...
See [the ACLs documentation][0] for more information on those properties.
...
ACL JSON or HuJSON (see [the ACLs documentation][0])
...
See [the ACLs documentation][0] for more information on the format of ACL tests.
